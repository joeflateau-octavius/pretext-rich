...
test('chat does not overflow at width 407', async ({ page }) => {
  await openDemo(page);
  await setChatWidth(page, 407);

  const railRight = await page.locator('#chatRenderContent').evaluate(el => el.getBoundingClientRect().right);
  const rightmostFragment = await page.locator('#chatRenderContent .render-line').evaluateAll(lines =>
    lines.flatMap(line => Array.from(line.children).map(child => child.getBoundingClientRect().right))
  );
  const maxRight = Math.max(...rightmostFragment);
  expect(maxRight).toBeLessThanOrEqual(railRight + 1);
});
